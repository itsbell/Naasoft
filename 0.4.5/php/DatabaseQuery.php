...
class InsertMenteeQuery extends DatabaseQuery
{
    ...
    public function Query(...$params)
    {
        ...
        CALL InsertToMentee(\"$params[0]\", \"$params[1]\",\"$params[2]\")
        ");
    }
}
...
class InsertApplyQuery extends DatabaseQuery
{
    ...
    {
        $this->connection->query("
        CALL InsertToApply(\"$params[0]\", \"$params[1]\", \"$params[2]\")
        ");
    }
}
...
class InsertPaymentQuery extends DatabaseQuery
{
    ...
    {
        $this->connection->query("
        CALL InsertToPayment(\"$params[0]\", \"$params[1]\", \"$params[2]\", $params[3])
        ");
    }
}
...
class GetMentoQuery extends DatabaseQuery
{
    ...
    public function Query(...$params)
    {
        $this->connection->query("CALL GetFromMento(\"$params[0]\", @name, @emailAddress)");
        $queryResult = $this->connection->query("SELECT @name, @emailAddress");

        $row = $queryResult->fetch_array();

        $mentoCard = new MentoCard();
        $mento = new Mento($row[0], $row[1]);
        $mentoCard->Add($mento);
