...
export class SolutionBook extends BusinessObjects {
    ...
    GetAllUntilNow(time) {
        ...
        while (i < this._length) {
            solutionList = this._objects[i];
            nowList = solutionList.GetAllUntilNow(time);
            nowBook.Add(nowList);
            i++;
        }
...
export class SolutionList extends BusinessObjects {
    ...
    GetAllUntilNow(time) {
        let nowList = new SolutionList();

        let solution;
        let i = 0;
        while (i < this._length) {
            solution = this._objects[i];
            if (solution.time.IsGreaterThan(time)) {
                nowList.Add(solution.Clone());
            }
            i++;
        }
