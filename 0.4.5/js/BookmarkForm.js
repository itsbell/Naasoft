...
export class BookmarkForm extends CompositeWindow {
    ...
    async OnLoaded() {
        ...
        if (workList.length === 0) {
            ...
            const date = `${String(apply.time.month).padStart(2, "0")}. ${String(apply.time.day).padStart(2, "0")} (${dayOfWeek})`;
            const time = `${ampm} ${String(hour).padStart(2, "0")}:${String(apply.time.minute).padStart(2, "0")}`;
            const dateTime = `${date}<br>${time}`;
